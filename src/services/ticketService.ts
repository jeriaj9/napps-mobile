...
export function mapBackendTicketToTicketProps(
  ...
  if (customFieldValues && customFieldValues.length > 0) {
    const dateField = customFieldValues.find(
      (f) => f.value && (f.value.includes('|') || f.value.includes(' to ') || f.value.includes('-'))
    );
    if (dateField) {
  ...
  const reqTypeName = requestTypeNameMap && requestTypeNameMap[ticket.request_type_id]
    ? requestTypeNameMap[ticket.request_type_id]
    : `Ticket #${ticket.ticket_number || ticket.id}`;

  const createdByInfo = ticket.created_by && employeesMap && employeesMap[ticket.created_by]
    ? employeesMap[ticket.created_by]
    : ticket.created_by
    ? { name: `Employee #${ticket.created_by}`, id: `NT-${ticket.created_by}` }
    : undefined;

  const ownerInfo = ticket.owner && employeesMap && employeesMap[ticket.owner]
    ? employeesMap[ticket.owner]
    : undefined;

  const assigneeInfo = ticket.assigned_to && employeesMap && employeesMap[ticket.assigned_to]
    ? employeesMap[ticket.assigned_to]
    : ticket.assigned_to
