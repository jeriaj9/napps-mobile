...
const getMediaSource = (imgName: string) => {
  if (imgName === 'design_system_feed_image') {
    return require('@/assets/images/design_system_feed_image.png');
  }
  return null;
};
...
const INITIAL_POSTS: FeedPost[] = [
  {
    id: 'post-sarah-chen',
    author: { name: 'Sarah Chen', role: 'Design Lead', initials: 'SC' },
    timeAgo: '2 hours ago',
    content: 'New design system launch! Excited to share this with the team.',
    media: ['design_system_feed_image'],
    likes: 45,
    commentsCount: 2,
    liked: false,
    ...
    comments: [
      {
        ...
        role: 'Frontend Dev',
        initials: 'S',
        content: "This looks incredible! Can't wait to start using it.",
        timeAgo: '1 hour ago',
      },
      {
        id: 'c-sarah-2',
        ...
        role: 'HR Manager',
        initials: 'A',
        content: 'Huge congratulations to the design team! 👏',
        timeAgo: '30 mins ago',
      }
    ],
    month: 'May 2026',
  ...
  },
  {
    ...
    author: { name: 'ANA SANCHEZ', role: 'HR Manager', initials: 'A' },
    timeAgo: '40 days ago',
    content: '⭐¡Un nuevo mes para celebrar la vida!⭐\n\nA todos nuestros cumpleañeros de Mayo, les deseamos un año lleno de logros, salud, risas and grandes aventuras....',
    media: ['bday_media_1'],
    likes: 8,
    commentsCount: 0,
    liked: false,
    reactions: { '👍': 8, '❤️': 0, '💡': 0, '🎉': 0, '🤔': 0 },
    comments: [],
    month: 'May 2026',
  },
];
...
export default function FeedScreen() {
  ...
  const renderPostItem = ({ item }: { item: FeedPost }) => {
    const mediaSrc = item.media.length > 0 ? getMediaSource(item.media[0]) : null;
      ...
      <View style={styles.postCard}>
        ...
        <View style={styles.postBody}>
          {mediaSrc ? (
            ...
            <View style={styles.mediaContainer}>
              {item.media.map((imgName) => (
                <View key={imgName} style={styles.mediaPlaceholder}>
                  <SymbolView name="photo" size={24} tintColor="#8E8E93" />
                  <ThemedText style={styles.mediaPlaceholderText}>Post Image</ThemedText>
                </View>
              ))}
            </View>
          ) : null}
        ...
        <View style={styles.postActions}>
          ...
          <Pressable style={styles.commentButton} onPress={() => setActiveCommentPost(item)}>
            <SymbolView name="bubble.left" size={16} tintColor="#60646C" />
            <ThemedText style={styles.commentButtonText}>{item.commentsCount}</ThemedText>
          </Pressable>
        </View>
      </View>
    );
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.headerComponentWrapper}>
            ...
            {/* Birthday Banner */}
            {isBirthdayToday(currentUser.birthday) && showBirthdayBanner && (
              <View style={[styles.birthdayBanner, { borderColor: theme.backgroundSelected }]}>
                <LinearGradient
                ...
                >
                  <View style={styles.birthdayBannerLeft}>
                    <SymbolView name="birthday.cake.fill" size={26} tintColor="#ffffff" />
                  </View>
                  <View style={styles.birthdayBannerMiddle}>
                    <ThemedText type="smallBold" style={styles.birthdayTitle}>
                      Happy Birthday, {currentUser.name}! 🎂🎉
                    </ThemedText>
                    <ThemedText style={styles.birthdaySubtitle}>
                      Wishing you a wonderful day filled with joy and celebration!
                    </ThemedText>
                  </View>
                  <Pressable
                    onPress={() => setShowBirthdayBanner(false)}
